    ...
    <div class="absolute bottom-0 left-1/2 transform -translate-x-1/2 -translate-y-1/3">
      <p class="text-red-300 bg-red-100 px-2 py-0.5 rounded-sm text-sm items-center whitespace-nowrap">
        ...
        <button v-if="show_label_left" @click="navigate_left()" class="whitespace-nowrap rounded-lg pl-1 pr-2">
          <svg class="inline" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><path class="text-red-300 fill-current"  d="M10.828 12l4.95 4.95-1.414 1.414L8 12l6.364-6.364 1.414 1.414z"/></svg>
          {{label_left}}
        </button> |
        <button @click="navigate_top()" class="whitespace-nowrap rounded-lg pl-1 pr-2">
          <svg class="inline" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><path class="text-red-300 fill-current" d="M12 10.828l-4.95 4.95-1.414-1.414L12 8l6.364 6.364-1.414 1.414z"/></svg>
          {{label_center}}
        </button> |
        <button v-if="show_label_right" @click="navigate_right()" class="whitespace-nowrap rounded-lg pl-1 pr-2">
          <svg class="inline" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><path class="text-red-300 fill-current"  d="M13.172 12l-4.95-4.95 1.414-1.414L16 12l-6.364 6.364-1.414-1.414z"/></svg>
          {{label_right}}
        </button>
      </p>
    </div>
